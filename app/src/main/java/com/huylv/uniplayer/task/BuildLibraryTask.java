...
public class BuildLibraryTask extends AsyncTask<Void, Void, Integer> {
    ...
    protected Integer doInBackground(Void... params) {
        ...
        for (File f : listFile) {
            ...
            mdr.release();
            Song song = new Song(name, artist, f.getAbsolutePath(), length, bitmap != null ? Config.BitMapToString(bitmap) : "");
    ...
    protected void onPostExecute(Integer integer) {
        ...
        if (Config.isOnline(context)) {
            ...
            } else if (listSongHasEmptyThumbnail()) {
                Log.e("cxz", "has false");
            } else {
            ...
            ((MainActivity) context).stopSyncAnim();
            ((MainActivity) context).refreshSongList();
            Toast.makeText(context, "No internet connection!" + Config.localSongList.size() + " in local", Toast.LENGTH_SHORT).show();
        }
    }

    private boolean listSongHasEmptyThumbnail() {
        for (Song s : Config.localSongList) {
            if (s.getBitmapString().equals("")) {
                return true;
            }
        }
        return false;
